  ...
  return (
    <ul className=''>
      {response.data.map(todo => {
        return (
          <li key={todo.id} className='flex justify-between items-center bg-gray-100 py-2 px-3 rounded-md space-x-2 my-1'>
            ...
            <button
              ...
              onClick={() => updateMutation.mutate({id: todo.id, isCompleted: !todo.isCompleted}, {onSuccess: () => {trpcContext.todo.list.invalidate()}})}
            >
              {todo.isCompleted ? 'Complete' : 'Incomplete'}
            </button>

            <button
              onClick={() => deleteMutation.mutate({id: todo.id}, {onSuccess: () => {trpcContext.todo.list.invalidate()}})}
